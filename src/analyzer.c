...
char *hier_extract(char *pnt, int levels)
{
    ...
    if ((!GLOBALS->hier_ignore_escapes) && (esc = strchr(pnt, '\\'))) {
        return ((levels == 1) ? esc
                              : pnt); /* shortcut out on escape IDs: level=1, esc char else all */
    }
    ...
    for (i = 0; i < len; i++) {
        ch = *(pnt2--);
        if ((only_nums_so_far) && (ch >= '0') &&
            (ch <= '9')) /* skip 1st set of signal.number hier from right if it exists */
        {
            continue;
            /* nothing */
        } else {
...
void CloseTrace(GwTrace *t)
{
    ...
    if (IsGroupBegin(t)) {
        t->flags |= TR_CLOSED;
        if (t->t_match) {
            t->t_match->flags |= TR_CLOSED;
        };

        if (!HasWave(t)) {
            /* Group End */
            if (t->t_match) {
                t->t_match->flags |= TR_COLLAPSED;
            };
        } else {
            /* Composite  End */
            if (t->t_match) {
...
static void AddTrace(GwTrace *t)
{
    ...
    } else if (GLOBALS->default_flags & TR_PTRANSLATED) {
    ...
    if (GLOBALS->default_flags & TR_TTRANSLATED) {
        ...
        if (t->t_filter) {
            ...
            t->flags &= ~TR_TTRANSLATED; /* malformed filter syntax?  should never have "which" of
                                            zero here */
        }
    }
...
int AddBlankTrace(char *commentname)
{
    ...
    flags_filtered = TR_BLANK | (GLOBALS->default_flags & (TR_CLOSED | TR_GRP_BEGIN | TR_GRP_END |
                                                           TR_COLLAPSED | TR_ANALOG_BLANK_STRETCH));
...
int AddVector(GwBitVector *vec, char *aliasname)
{
    ...
    if (!vec)
        return (0); /* must've passed it a null pointer by mistake */
...
void RemoveTrace(GwTrace *t, int dofree)
{
    ...
    if (t == GLOBALS->traces.first) {
        ...
        if (t->t_prev) {
            ...
            /* this code should likely *never* execute as if( t == GLOBALS->traces.first ) above
             * should catch this */
...
GwTrace *CutBuffer(void)
{
    ...
    if (!t)
        return (NULL); /* keeps a double cut from blowing out the buffer */
...
GwTrace *PasteBuffer(void)
{
    ...
    if (GLOBALS->traces.buffer) {
        ...
        while (t) {
            ...
            t = t->t_next;
            count++;
            if (count == GLOBALS->traces.buffercount)
                break;
        }
    }
...
int TracesReorder(int mode)
{
    ...
    switch (mode) {
        case TR_SORT_INS:
            cptr = traceinamecompare;
            break;
        case TR_SORT_NORM:
            cptr = tracenamecompare;
            break;
        case TR_SORT_LEX:
            cptr = tracesignamecompare;
            break;
        default:
            cptr = NULL;
            break;
        ...
        for (i = 0; i < GLOBALS->traces.total; i++) {
            if (tsort[i]->flags & TR_GRP_BEGIN) {
                ...
                for (j = i; j < GLOBALS->traces.total; j++) {
                    if (tsort[j]->flags & TR_GRP_BEGIN) {
                        cnt++;
                    } else if (tsort[j]->flags & TR_GRP_END) {
                        cnt--;
                    }
...
void UpdateTraceSelection(GwTrace *t)
{
    ...
    } else if ((t->t_grp) && IsSelected(t->t_grp)) {
        t->flags |= TR_HIGHLIGHT;
    } else if ((t->flags & (TR_BLANK | TR_ANALOG_BLANK_STRETCH)) &&
        ...
        if (!(t->flags & TR_HIGHLIGHT)) {
            ...
            while ((tscan) &&
                   (tscan = tscan->t_prev)) /* && branch formerly was (tscan =
                                               GivePrevTraceSkipUpdate(tscan, 1)): overkill as blank
                                               traces in transactions don't nest into groups */
            {
...
void EnsureGroupsMatch(void)
{
    ...
    if ((underflow_sticky) || (oc_cnt > 0)) {
        ...
        while (t) {
            t2 = t->t_next;
            RemoveTrace(t, 0); /* conservatively don't set "dofree", if there is a reload memory
                                  will reclaim */
            t = t2;
        }
    }
