...
GwTrace *Node_to_Trptr(GwNode *nd)
{
    ...
    fprintf(stderr, "TCL_SUPPORT: Node %p, name: %s, mv.mvlfac = %p, mv.mvlfac_vlist = %p\n", 
            nd, nd->nname ? nd->nname : "NULL", nd->mv.mvlfac, nd->mv.mvlfac_vlist);
    if (nd->mv.mvlfac_vlist) {
        fprintf(stderr, "TCL_SUPPORT: Using modern dump file API for VCD\n");
        ...
        fprintf(stderr, "TCL_SUPPORT: Using old import_trace for FST/LX2\n");
        import_trace(nd);
    } else if (GLOBALS->dump_file && GW_IS_DUMP_FILE(GLOBALS->dump_file)) {
        fprintf(stderr, "TCL_SUPPORT: Using modern dump file API (fallback)\n");
        /* Use modern dump file API as fallback */
        GwNode *nodes[2] = {nd, NULL};
        gw_dump_file_import_traces(GLOBALS->dump_file, nodes, NULL);
    } else {
        fprintf(stderr, "TCL_SUPPORT: No valid import method found for node\n");
    }
