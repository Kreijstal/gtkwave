...
static void feed_shm_data(GwSharedMemory *shm, const char *data) {
    static gssize produce_offset = 0;
...
static void test_interactive_vcd_from_file(void) {
    ...
    while (fgets(line_buffer, sizeof(line_buffer), vcd_file)) {
        feed_shm_data(shm, line_buffer);
        g_usleep(1000); // Simulate slow stream

        if (!header_loaded && strstr(line_buffer, "$enddefinitions")) {
            g_test_message("Header detected. Performing initial load.");
            ...
            bit_node = get_node_by_name(dump_file, "variables.bit");
            g_assert_nonnull(bit_node);

            header_loaded = TRUE;
            expected_hist_count = 2; // Expect sentinel head + initial 'x' entry
        }
        else if (header_loaded && line_buffer[0] == '#') {
            ...
            // 1. Kick loader to parse SHM data into compressed vlist
            gw_vcd_partial_loader_kick(loader);

            // 2. Import traces to convert vlist to GwHistEnt linked list
            GwNode *nodes_to_import[] = { bit_node, NULL };
            gw_dump_file_import_traces(dump_file, nodes_to_import, &error);
            g_assert_no_error(error);

            // 3. Invalidate and rebuild harray (simulating the fix)
