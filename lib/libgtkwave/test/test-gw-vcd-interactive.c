...
static GwNode* get_node_by_name(GwDumpFile *dump_file, const char* name) {
    ...
    for (gint i = 0; i < num_facs; i++) {
        ...
        if (s && s->n && g_strcmp0(s->n->nname, name) == 0) return s->n;
    }
    return NULL;
}
...
static void feed_shm_data(GwSharedMemory *shm, const char *data) {
    ...
    memcpy(shm_data + produce_offset + 5, data, len);
    put_8(shm_data + produce_offset, 1);
...
static void test_interactive_vcd_from_file(void) {
    ...
    while (fgets(line_buffer, sizeof(line_buffer), vcd_file)) {
        ...
        if (!header_loaded && strstr(line_buffer, "\$enddefinitions")) {
            ...
            if (current_time > 0) expected_hist_count++;
